"""
FastMCP server exposing Aperio public web search tools.

The server is launched over stdio by langchain-mcp-adapters. It writes evidence
and per-run budget state under APERIO_OUTPUTS_DIR.
"""
...
def _write_evidence(save_path: str, payload: dict[str, Any]) -> None:
    ...
    if not str(target).lower().startswith(str(outputs_root).lower()):
        raise ValueError("save_path resolves outside APERIO_OUTPUTS_DIR")
...
def internet_search(query: str, max_results: int = 3, save_path: str = "") -> str:
    """Search public web results with DuckDuckGo and optionally save JSON evidence under /outputs/."""
    ...
    if save_path:
        if save_path in PRD_REVIEW_SEARCH_BUDGET:
            search_counts = _read_counts()
            search_counts[save_path] = search_counts.get(save_path, 0) + 1
            _write_counts(search_counts)
        try:
            ...
            return _json(
                {
                    ...
                    "query": query,
                    "save_path": save_path,
                    "error": f"failed to save evidence: {type(exc).__name__}: {exc}",
                    "results": results,
                }
            )
